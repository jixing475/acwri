...
acwri_theme <- function() {
  list(
    ...
    # historically, acwri has used yellow for this
...
# silence -----------------------------------------------------------------
#' Suppress acwri's messaging
#'
#' Execute a bit of code without acwri's normal messaging.
#'
...
ui_silence <- function(code) {
  withr::with_options(list(acwri.quiet = TRUE), code)
}

is_quiet <- function() {
  isTRUE(getOption("acwri.quiet", default = FALSE))
}
...
acwri_map_cli <- function(x, ...) UseMethod("acwri_map_cli")

#' @export
acwri_map_cli.default <- function(x, ...) {
  ui_abort(c(
    "x" = "Don't know how to {.fun acwri_map_cli} an object of class
           {.obj_type_friendly {x}}."
  ))
}

#' @export
acwri_map_cli.NULL <- function(x, ...) NULL

#' @export
acwri_map_cli.character <- function(
  x,
  template = "{.val <<x>>}",
  .open = "<<",
  .close = ">>",
