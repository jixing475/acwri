  ...
  c(
    if (!on_cran) {
      c(
        "First release:",
        "",
        todo("`acwri::use_news_md()`", !has_news),
        todo("`acwri::use_cran_comments()`"),
    ...
    todo("`acwri::use_news_md()`", on_cran && !has_news),
    todo(
    ...
    todo("`acwri::use_github_release()`"),
    todo("`acwri::use_dev_version(push = TRUE)`"),
    todo("`acwri::use_news_md()`", !has_news),
...
get_release_news <- function(
  ...
  if (is.null(news)) {
    ui_bullets(c(
      ...
      "i" = "{.pkg acwri} consults this file for release notes.",
      "i" = "Call {.run acwri::use_news_md()} to set this up for the future."
    ))
