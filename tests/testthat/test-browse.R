test_that("github_url() errors if no project", {
  ...
  expect_acwri_error(github_url(), "not.*inside a valid project")
})
...
test_that("github_url() works on active project", {
  ...
  expect_acwri_error(github_url(), "no DESCRIPTION")
  expect_acwri_error(github_url(), "no GitHub remotes")
...
test_that("github_url() errors for nonexistent package", {
  expect_acwri_error(github_url("1234"), "Can't find")
})
...
test_that("browse_XXX() goes to correct URL", {
  ...
  expect_equal(
    browse_cran("acwri"),
    "https://cran.r-project.org/package=acwri"
  )
})
...
test_that("browse_package() errors if no project", {
  withr::local_dir(path_temp())
  local_project(NULL, force = TRUE, setwd = TRUE)
  expect_acwri_error(browse_project(), "not.*inside a valid project")
})
