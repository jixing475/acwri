...
#' acwri consults the following sources, in this order, to set `DESCRIPTION`
#' fields:
#' * `fields` argument of [create_package()] or `use_description()`
#' * `getOption("acwri.description")`
#' * Defaults built into acwri
#'
#' The fields discovered via options or the acwri package can be viewed with
#' `use_description_defaults()`.
#'
#' If you create a lot of packages, consider storing personalized defaults as a
#' named list in an option named `"acwri.description"`. Here's an example of
...
#' ```
#'
#' Prior to acwri v2.0.0, `getOption("devtools.desc")` was consulted for
#' backwards compatibility, but now only the `"acwri.description"` option is
#' supported.
#'
...
#'   `"acwri.description"` option or the acwri package (in that order), and
...
use_description_defaults <- function(
  ...
  acwri <- acwri_description_defaults(package)
  ...
  if (roxygen) {
    ...
    acwri$Roxygen <- "list(markdown = TRUE)"
    acwri$RoxygenNote <- roxygen_note
  }

  options <- getOption("acwri.description") %||% list()
  ...
  defaults <- modify_this(acwri, options)
...
acwri_description_defaults <- function(package = NULL) {
  list(
