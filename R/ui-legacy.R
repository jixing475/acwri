...
#'   These functions are now superseded. External users of the `acwri::ui_*()`
...
#'   `acwri::ui_*()` functions were created, but it has had that for a while
#'   now and it's the superior option. There is even a cli vignette about how to
#'   make this transition: `vignette("acwri-ui", package = "cli")`.
#'
#'   acwri itself now uses cli internally for its UI, but these new functions
#'   are not exported and presumably never will be. There is a developer-focused
#'   article on the process of transitioning acwri's own UI to use cli:
#'   [Converting acwri's UI to use cli](https://acwri.r-lib.org/articles/ui-cli-conversion.html).
...
#' by setting `options(acwri.quiet = TRUE)`. Use [ui_silence()] to silence
#' selected actions.
#'
...
ui_stop <- function(x, .envir = parent.frame()) {
  ...
  cnd <- structure(
    class = c("acwri_error", "error", "condition"),
    list(message = x)
  )
...
#' `ui_yeah()` and `ui_nope()` are no longer used internally in acwri.
#'
...
ui_yeah <- function(
  ...
  # suppressed when `acwri.quiet = TRUE`, i.e. if the menu() appears, then
...
# can be quieted with 'acwri.quiet' when needed.
ui_inform <- function(...) {
  if (!is_quiet()) {
    inform(paste0(...))
  }
