...
Many acwri functions act on the **active project**, the path to which is stored in the internal environment `proj`, specifically in `proj$cur`.
...
-   A package that imported acwri would also need to set project on attach.
-   Currently user can open R, attach (or load) acwri, and then change working directory to their target project. As long as they are in the project the first time `proj_get()` is called, the correct project will be made active. This is not our preferred workflow, but it is common.

## Helper functions

With some ambivalence, internally-oriented helpers like `write_union()` are exported.
This helps developers who are extending acwri to create a package to standardize project setup within their own organization.
...
acwri relies on fs for file system operations.
...
This is so we can control verbosity package-wide with the `acwri.quiet` option, which defaults to `FALSE`.

-   Exploited in acwri tests: option is set and unset in `setup.R` and `teardown.R`, to suppress output by default.
-   Individual tests that need output, e.g. for snapshot testing, should include `withr::local_options(acwri.quiet = FALSE)`.
-   Other packages can muffle a acwri call through similar means.
-   `ui_silence()` is a convenience helper for executing small bits of code silently.

Note that acwri exports many `ui_*()` functions for historical reasons, all defined in `ui-legacy.R`, and all marked as superseded now.
See the developer-facing article "Converting acwri's UI to use cli" for more.
