...
if (!is.null(session_temp_proj)) {
  ...
  ui_bullets(c(
    ...
    bulletize(acwri_map_cli(Rproj_files)),
  ...
  ui_silence(
    switch(
      ...
      package = create_package(
        ...
        # When the active acwri project is a package created with this
        # function, testthat learns its edition from *that* package, not from
        # acwri. So, by default, opt in to testthat 3e in these ephemeral test
        # packages.
...
expect_acwri_error <- function(...) {
  expect_error(..., class = "acwri_error")
}
