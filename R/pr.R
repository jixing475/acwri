...
#'
#' The [Pull Request
#' Helpers](https://acwri.r-lib.org/articles/articles/pr-functions.html)
...
pr_init <- function(branch) {
    ...
    if (cfg$type %in% maybe_good_configs) {
      ui_bullets(c(
        ...
        "i" = "You probably need to configure a personal access token for
               {.val {tr$host}}.",
        "i" = "See {.run acwri::gh_token_help()} for help with that."
      ))
  ...
  gert::git_config_set(config_key, value = "acwri::pr_init", repo = repo)

  ui_bullets(c("_" = "Use {.run acwri::pr_push()} to create a PR."))
  invisible()
}
...
pr_resume <- function(branch = NULL) {
  ...
  if (!gert::git_branch_exists(branch, local = TRUE, repo = repo)) {
    code <- glue('acwri::pr_init("{branch}")')
    ui_abort(c(
  ...
  ui_bullets(c("_" = "Use {.run acwri::pr_push()} to create or update PR."))
  invisible()
}
...
pr_fetch <- function(number = NULL, target = c("source", "primary")) {
  ...
  if (nrow(remote) == 0) {
    ...
    gert::git_config_set(config_key, "acwri::pr_fetch", repo = repo)
  }
...
pr_push <- function() {
  ...
  if (is.null(pr)) {
    ...
    ui_bullets(c(
      "_" = "View PR at {.url {pr$pr_html_url}} or call {.run acwri::pr_view()}."
    ))
  }
...
pr_clean <- function(
  ...
  # delete remote, if we (acwri) added it AND no remaining tracking branches
  created_by <- git_cfg_get(glue("remote.{pr$pr_remote}.created-by"))
  if (is.null(created_by) || !grepl("^acwri::", created_by)) {
    return(invisible())
  }
...
choose_pr <- function(tr = NULL, pr_dat = NULL) {
  ...
  if (n_not_shown > 0) {
    ...
    info2 <- cli::format_inline(
      "Call {.run acwri::browse_github_pulls()} to browse all PRs."
    )
    prompt <- glue("{prompt}\n{info1}\n{info2}")
  }
...
check_pr_branch <- function(default_branch) {
  ...
  check_current_branch(
    ...
    message = c(
      ...
      "i" = "Do you need to call {.fun acwri::pr_init} (new PR)?
             Or {.fun acwri::pr_resume} or
             {.fun acwri::pr_fetch} (existing PR)?"
    )
  )
}
