...
acwri can help you with many of the Git and GitHub tasks that arise when managing R projects and packages.
...
Our goal is a setup that works well for acwri **and** for other tools you may be using, such as command line Git and Git clients (including, but not limited to, RStudio).
...
#| fig-align: center
#| out-width: 80%
#| fig-alt: Diagram showing different ways of interacting with GitHub as a server and
#|   the credential needed for each method
...
The diagnostic functions `acwri::gh_token_help()` and `acwri::git_sitrep()` will offer some of the advice you see here, but directly in your R session.
...
acwri defaults to HTTPS in functions like `create_from_github()` and `use_github()`, as of v2.0.0[^acwri-https-default].

[^acwri-https-default]: As always, the `acwri.protocol` option can be configured to customize your own default.
...
You will have the [gitcreds package](https://r-lib.github.io/gitcreds/) installed, as of acwri v2.0.0, because acwri uses gh, and gh uses gitcreds.
...
`acwri::edit_r_environ()` can be helpful for getting `.Renviron` open for editing.
Don't forget to restart R for this change to take effect!

You can check that your PAT has been successfully stored with `acwri::gh_token_help()` **executed in a fresh R session**:
...
The more general function `acwri::git_sitrep()` will also report on your PAT, along with other aspects of your Git/GitHub setup.
...
Most users should be ready to work with Git and GitHub from R now, specifically with gert, gh, and acwri.
In this section, we cover more specialized topics that only apply to certain users.

### GitHub Enterprise

As of v2.0.0, acwri should fully support GitHub Enterprise deployments.
If you find this is not true, please [open an issue](https://github.com/jixing475/acwri/issues/new).

There are a few acwri functions that support an explicit `host` argument, but in general, acwri honors the GitHub host implicit in URLs, e.g., the locally configured Git remotes, or inherits the default behaviour of gh.
The gh package honors the `GITHUB_API_URL` environment variable which, when unset, falls back to `https://api.github.com`.

In general, acwri, gh, and gitcreds should all work with GitHub Enterprise, as long as the intended GitHub host is discoverable or specified.
...
```{r}
#| eval: false
acwri::gh_token_help("https://github.acme.com")
# or
acwri::git_sitrep()
```
...
If you have any doubt about your previous practices, open `.Renviron`, look for any line setting the `GITHUB_PAT` environment variable, and delete it. `acwri::edit_r_environ()` can be helpful for getting `.Renviron` open for editing.
Don't forget to restart R for this change to take effect.
...
If you still need to use `.Renviron` method, `acwri::edit_r_environ()` opens that file for editing.

```{r}
#| eval: false
acwri::edit_r_environ()
```
...
```{r}
#| eval: false
remotes::install_github("jixing475/acwri")

pak::pkg_install("jixing475/acwri")
```
...
pak uses the same PAT-finding approach as acwri/gh/gitcreds.
