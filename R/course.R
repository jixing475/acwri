...
#'   stored in the global option `acwri.destdir`, if defined, or to the user's
...
#' @examples
#' \dontrun{
#' # download the source of acwri from GitHub, behind a bit.ly shortlink
#' use_course("bit.ly/acwri-shortlink-example")
#' use_course("http://bit.ly/acwri-shortlink-example")
#'
...
#'   `destdir` or configure the `"acwri.destdir"` option to prevent this.
...
#' @export
use_course <- function(url, destdir = getOption("acwri.destdir")) {
...
#'   url,
#'   # conspicuous_place() = `getOption('acwri.destdir')` or desktop or home
...
conspicuous_place <- function() {
  destdir_opt <- getOption("acwri.destdir")
