...
Various functions in acwri insert a badge into a package's README file.
...
<!-- badges: start -->
[![R-CMD-check](https://github.com/jixing475/acwri/actions/workflows/R-CMD-check.yaml/badge.svg)](https://github.com/jixing475/acwri/actions/workflows/R-CMD-check.yaml)
[![Codecov test coverage](https://codecov.io/gh/jixing475/acwri/branch/main/graph/badge.svg)](https://app.codecov.io/gh/jixing475/acwri?branch=main)
<!-- badges: end -->
...
There are at least two ways that acwri can influence this:
...
* The image itself. We can encourage the badge providers to build accessibility
  into their badge SVGs. If the "native" badge is not accessible, acwri can
...
acwri currently inserts badges in a special "badge block" in the README, fenced with HTML comment tags.
...
acwri definitely controls this `alt_text`, so that is something I'd like feedback on.
...
The badge is inside an HTML comment block, like acwri does for README badges.
...
Finally, and most relevant to acwri, shields.io offers pre-configured dynamic badges for the most commonly "badged" services, including GitHub Actions and Codecov.

Here is a shields.io badge for acwri's `R CMD check` workflow on GitHub Actions:

<!-- badges: start -->
[![my_alt_text-R-CMD-check](https://img.shields.io/github/workflow/status/jixing475/acwri/R-CMD-check?label=my_label-R-CMD-check)](https://github.com/jixing475/acwri/actions/workflows/R-CMD-check.yaml)
<!-- badges: end -->

Again, I am indicating fields I control with `my_alt_text` and `my_label`, so it's easier to get feedback on what acwri should do.

Here is the `title` and `aria-label` of the badge above:

```{r}
inspect_badge("https://img.shields.io/github/workflow/status/jixing475/acwri/R-CMD-check?label=my_label-R-CMD-check")
```

Now we will take inventory of the main badges inserted by acwri and where things stand re: accessibility.
...
```{r}
inspect_badge("https://github.com/jixing475/acwri/actions/workflows/R-CMD-check.yaml/badge.svg")
```
...
A maintainer could do this as a one-off or, if the GitHub badge upgrade is slow in coming, we could implement it in `acwri::use_github_actions_badge()`.

## Package lifecycle

`acwri::use_lifecycle_badge()` declares the developmental stage of a package, using the framework from <https://lifecycle.r-lib.org/articles/stages.html>.
...
It's possible that acwri should be using a badge "owned" by the lifecycle package.
...
`acwri::use_cran_badge()` places a badge that indicates what package version is available on CRAN.
...
Here's the badge placed by `acwri::use_cran_badge()`:

<!-- badges: start -->
[![CRAN status](https://www.r-pkg.org/badges/version/acwri)](https://CRAN.R-project.org/package=acwri)
<!-- badges: end -->

Here is the `title` and `aria-label` of the badge above:

```{r}
inspect_badge("https://www.r-pkg.org/badges/version/acwri")
```
...
`acwri::use_coverage(type = c("codecov", "coveralls", ...)` calls the internal helper `acwri:::use_codecov_badge()` to insert a badge for either the Codecov or Coveralls service.

Here are examples of those two badges (note that acwri does not use Coveralls, so I'm using a different package to demo):
...
```{r}
inspect_badge("https://codecov.io/gh/jixing475/acwri/branch/main/graph/badge.svg")
...
<!-- badges: start -->
[![Codecov test coverage](https://img.shields.io/codecov/c/github/jixing475/acwri?label=test%20coverage&logo=codecov)](https://app.codecov.io/gh/jixing475/acwri?branch=main)
...
```{r}
inspect_badge("https://img.shields.io/codecov/c/github/jixing475/acwri?label=test%20coverage&logo=codecov")
...
If a package is on Bioconductor, `acwri::use_bioc_badge()` can be used to insert a badge for its Bioconductor build status.
