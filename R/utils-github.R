  ...
  if (isTRUE(github_get) && !all(grl$github_got)) {
    ...
    ui_abort(c(
      ...
      "Otherwise, you probably need to configure a personal access token (PAT)
       for {.val {oops_hosts}}.",
      "See {.run acwri::gh_token_help()} for advice."
    ))
  }
...
#'
#' Most acwri functions should call the higher-level functions `target_repo()`
#' or `target_repo_spec()`.
#'
...
stop_bad_github_remote_config <- function(cfg) {
  ui_abort(
    ...
    class = "acwri_error_bad_github_remote_config",
    cfg = cfg
  )
}
...
stop_maybe_github_remote_config <- function(cfg) {
  ...
  ui_abort(
    message = unlist(msg),
    class = "acwri_error_invalid_pr_config",
    cfg = cfg
  )
}
...
read_more <- function() {
  c(
    "i" = "Read more about the GitHub remote configurations that acwri supports at:",
  ...
  utils::modifyList(
    ...
    list(
      ...
      desc = c(
        ...
        "i" = "{.fun acwri::create_from_github} can do this.",
        read_more()
      )
    )
  )
    ...
    list(
      ...
      desc = c(
        ...
        "i" = "We may be offline or you may need to configure a GitHub personal
               access token.",
        "i" = "{.run acwri::gh_token_help()} can help with that.",
        read_more_maybe()
      )
    )
  )
