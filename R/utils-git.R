...
check_uses_git <- function() {
  ...
  ui_abort(c(
    ...
    "Do you need to run {.run acwri::use_git()}?"
  ))
}
...
# translate from "acwri" terminology to "git" terminology
...
git_ask_commit <- function(message, untracked, push = FALSE, paths = NULL) {
  ...
  paths <- sort(paths)
  ui_paths <- acwri_map_cli(paths, template = '{.path {pth("<<x>>")}}')
...
git_conflict_report <- function() {
  ...
  conflicted_paths <- acwri_map_cli(
    conflicted,
    template = '{.path {pth("<<x>>")}}'
  )
