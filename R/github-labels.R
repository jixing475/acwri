...
#' areas of functionality. For example, acwri has labels like "description",
...
use_github_labels <- function(
  ...
  if (all(labels %in% cur_label_names)) {
    ...
    ui_bullets(c(
      ...
      bulletize(acwri_map_cli(to_add))
    ))
  ...
  if (identical(cur_label_colours[names(colours)], colours)) {
    ...
    ui_bullets(c(
      "v" = "Updating colours:",
      bulletize(acwri_map_cli(to_update))
    ))
  ...
  if (delete_default) {
    ...
    if (length(to_remove) > 0) {
      ui_bullets(c(
        ...
        bulletize(acwri_map_cli(to_remove))
      ))
