...
* acwri's criteria for recognizing a project (listed in `?proj_utils`) have
...
* `use_course()` and `use_zip()` open the new folder in a new session of the
  ...
  the folder doesn't already fulfill any of acwri's criteria for a project
  root folder (#2127).
...
* `create_quarto_project()` is a new experimental function that combines basic
  ...
  acwri's `create_*()` functions, such as opening the newly created project in
...
* `use_tidy_upkeep_issue()` records the year it is being run in the
  `Config/acwri/upkeep` field in DESCRIPTION. If this value exists, it is
...
* The `ui_*()` functions have been marked as
  ...
  `acwri::ui_*()` functions were first created, but it does now and it's the
  superior option.
  There is a cli vignette about how to make this transition:
  `vignette("acwri-ui", package = "cli")`.
  acwri no longer uses the `ui_*()` functions internally, in favor of new
  cli-based helpers that are not exported.

## Deprecated function and argument removal

We are removing functions and arguments that were deprecated as of acwri
...
* `acwri::use_version()` now tolerates empty lines preceding the
...
acwri has a more sophisticated understanding of the default branch and gains several functions to support default branch renaming.
...
* `DESCRIPTION` files generated by acwri no longer include `LazyData` by
...
* `"acwri.overwrite"` is a new option. When set to `TRUE`, acwri overwrites
...
acwri has various functions that help with Git-related tasks, which break down into two categories:
...
[Managing Git(Hub) Credentials](https://acwri.r-lib.org/articles/articles/git-credentials.html)

The main user-facing changes in acwri are:

* acwri should discover and use the same credentials as command line Git.
* acwri should be able to work with any GitHub deployment. While github.com is the default, GitHub Enterprise deployments are fully supported. The target GitHub host is determined from the current project's configured GitHub remotes, whenever possible.
...
acwri gains a more formal framework for characterizing a GitHub remote configuration. We look at:
...
This is an internal matter, but users will notice that acwri is more clear about which configurations are supported by various functions and which are not. The most common configurations are reviewed in a [section of Happy Git](https://happygitwithr.com/common-remote-setups.html).

When working in a fork, there is sometimes a question whether to target the fork or its parent repository. For example, `use_github_links()` adds GitHub links to the URL and BugReports fields of DESCRIPTION. If someone calls `use_github_links()` when working in a fork, they probably want those links to refer to the *parent* or *source* repo, not to their fork, because the user is probably preparing a pull request. acwri should now have better default behaviour in these situations and, in some cases, will present an interactive choice.

## Default branch

There is increasing interest in making the name of a repo's default branch configurable. Specifically, `main` is emerging as a popular alternative to `master`. acwri now discovers the current repo's default branch and uses that everywhere that, previously, we had hard-wired `master`.
...
The default Git protocol is now "https" and we no longer provide an interactive choice, by default, in interactive sessions. As always, a user can express a preference for "ssh" in individual function calls, for an R session via `use_git_protocol()`, and for all R sessions via the `acwri.protocol` option (#1262).
...
acwri is now fully cognizant of the [changes to RStudio preferences](https://posit.co/blog/rstudio-1-3-preview-configuration/) in RStudio 1.3:
...
The legacy `"devtools.desc"` option is no longer consulted when populating a new DESCRIPTION file. You must use the `"acwri.description"` now (#1069).
...
acwri has been re-licensed as MIT (#1252, #1253).
...
Patch release to refactor usage of withr in the tests for forward compatibility with an upcoming withr release. All changes are within the acwri tests.
...
* `use_rstudio()` now sets the `LineEndingConversion` to `Posix` so that
  packages created using acwri always use LF line endings, regardless of
  who contributes to them (#1002).

* In the `acwri.description` option, you can now set `Authors@R = person()`
...
* A new article [Pull request helpers](https://acwri.r-lib.org/articles/articles/pr-functions.html)
...
* When writing files, acwri now respects line endings. Default line endings
...
* `acwri.destdir` is a new option that is consulted when deciding where to
...
acwri gains several functions to inspect and manipulate the Git situation for the current project = repository. We also provide more control and visibility
into git2r's workings, especially around credentials (acwri uses git2r for all
Git operations).
...
* `git_protocol()` + `use_git_protocol()` and `git_credentials()` +
  ...
  (#653). acwri honors the `acwri.protocol` option, which allows you to
...
acwri gains tooling to manage part of a file. This is currently used for
...
*   `use_tibble()` and `use_rcpp()` automatically add roxygen tags to
    ...
    workflows, but I suspect the set of people who use acwri and Rcpp but
    not roxygen2 is very small.

## Extending and wrapping acwri
...
* `proj_get()` and `proj_set()` no longer have a `quiet` argument. The
  ...
  as other messages, i.e. `getOption("acwri.quiet", default = FALSE)` (#441).

* A new set of `ui_*()` functions makes it possible to give your own code
  the same user interface as acwri (#308). All use the glue and crayon and
...
* `with_project()` and `local_project()` are new withr-style functions to
  temporarily set an active acwri project. They make acwri functions easier
...
All acwri file system operations now use the [fs](https://fs.r-lib.org) package (#177). This should not change how acwri functions, but users may notice these features of fs-mediated paths:
...
## Extending or wrapping acwri

These changes make it easier for others to extend acwri, i.e. to create workflow packages specific to their organization, or to use acwri in other packages.
...
build paths within it (#415, #425).
  ...
  * `edit_file()`, `write_over()`, and `write_union()` are newly exported helpers. They are mostly for internal use, but can also be useful in packages that extend or customize acwri (#344, #366, #389).
...
* `getOption("acwri.quiet", default = FALSE)` is consulted when printing user-facing messages. Set this option to `TRUE` to suppress output, e.g., to use acwri functions quietly in another package. For example, use `withr::local_options(list(acwri.quiet = TRUE))` in the calling function (#416, #424).

## New functions

* `proj_sitrep()` reports current working directory, the active acwri project, and the active RStudio Project. Call this function if things seem weird and you're not sure what's wrong or how to fix it. Designed for interactive use and debugging, not for programmatic use (#426).
...
* The logic for setting DESCRIPTION fields in `create_package()` and `use_description()` got a Spring Cleaning. Fields directly specified by the user take precedence, then the named list in `getOption("acwri.description")` is consulted, and finally defaults built into acwri. `use_description_defaults()` is a new function that reveals fields found in options and built into acwri. Options specific to one DESCRIPTION field, e.g. `devtools.desc.license`, are no longer supported. Instead, use a single named list for all fields, preferably stored in an option named `"acwri.description"` (however,`"devtools.desc"` is still consulted for backwards compatibility). (#159, #233, #367)
...
* acwri has a website: <https://acwri.r-lib.org> (#217). It includes an article with advice on system setup, for acwri and for R development more generally.
...
acwri is gradually evolving towards supporting more general R "projects", not just packages. This is still a work in progress, so please let me know if you use a function that you think should work with projects but doesn't. You can also try out the new `create_project()` which creates a basic RStudio project.
...
The output from all acwri commands has been reviewed to be informative but not overwhelming. acwri takes advantage of colour (using crayon and RStudio 1.1) to help chunk the output and clearly differentiate what you need to do vs. what has been done for you.
...
* `use_acwri()` opens your `.Rprofile` and gives you the code to copy
  and paste in.
