test_that("use_package() won't facilitate dependency on tidyverse/tidymodels", {
  ...
  expect_acwri_error(use_package("tidyverse"), "rarely a good idea")
  expect_acwri_error(use_package("tidymodels"), "rarely a good idea")
})
...
test_that("package_remote() errors if no remote and no github URL", {
  ...
  expect_acwri_error(package_remote(d), "Cannot determine remote")
})
