...
#' (PAT) is needed for certain tasks acwri does via the GitHub API, such as
...
#'   such as acwri, gert, and gh can automatically retrieve that PAT and use
...
#' Git(Hub)
#' Credentials](https://acwri.r-lib.org/articles/articles/git-credentials.html)
#'
#' @details
...
create_github_token <- function(
  ...
  if (is_linux()) {
    message <- c(
      ...
      "i" = "Read more in the {.href ['Managing Git(Hub) Credentials' article](https://acwri.r-lib.org/articles/articles/git-credentials.html)}."
    )
  }
  message <- c(
    message,
...
pat_sitrep <- function(
  ...
  if (!have_pat) {
    ...
    hint <- code_hint_with_host("acwri::create_github_token", host, "host")
    ui_bullets(c(
      "_" = "To create a personal access token, call {.run {hint}}."
    ))
    hint <- code_hint_with_host("gitcreds::gitcreds_set", host)
    url <- "https://acwri.r-lib.org/articles/articles/git-credentials.html"
    ui_bullets(c(
...
scold_for_renviron <- function() {
  ...
  ui_bullets(c(
    ...
    "i" = "For most use cases, it is better to NOT define the PAT in
           {.file .Renviron}.",
    "_" = "Call {.run acwri::edit_r_environ()} to edit that file.",
...
scold_for_scopes <- function(scopes) {
  ...
  message <- c(
    ...
    "i" = "Tokens initiated with {.fun acwri::create_github_token} default to the
           recommended scopes."
  )
  ui_bullets(message)
}
