---
title: "Converting acwri's UI to use cli"
---
...
asciicast::init_knitr_engine(
  ...
  startup = quote({
    ...
    ui_code_snippet <- acwri:::ui_code_snippet
    acwri_theme <- acwri:::acwri_theme
    ui_inform <- acwri:::ui_inform
    kv_line <- acwri:::kv_line
    ui_special <- acwri:::ui_special
    ui_abort <- acwri:::ui_abort
    ui_bullets <- acwri:::ui_bullets
    set.seed(1) })
)
```
...
Another important feature is that all of this output can be turned off package-wide via the `acwri.quiet` option.

```{asciicast}
withr::with_options(
  list(acwri.quiet = TRUE),
  ui_info("You won't see this message.")
)
withr::with_options(
  list(acwri.quiet = FALSE), # this is the default
...
* Early exit, without emitting messages, if the `acwri.quiet` option is `TRUE`.
* A acwri theme, that changes the color of certain bullet styles and adds a new style for todo's.
...
ui_bullets(c(
  ...
  "No indentation at all. Not used much in acwri."
))
```
...
`is_quiet()` just consults the `acwri.quiet` option and implements the default of `FALSE`.
...
It's just a `withr::with_*()` function for executing `code` with `acwri.quiet = TRUE`.
...
acwri has its own inline styles (mostly) for use inside functions like `ui_todo()`:
...
-   `ui_field()` becomes `{.field blah}`. In `acwri_theme()`, I tweak the `.field` style to apply single quotes if color is not available, which is what `ui_field()` has always done.
...
The main point of `ui_abort()` is to use to `cli_abort()` (and to continue applying the `"acwri_error"` class).

I also use `ui_abort()` to apply different default bullet naming/styling.
Starting with `"x"` and then defaulting to `"i"` seems to fit best with acwri's existing errors.
...
Sidebar: Now that I'm looking at a lot of the new errors with `ui_abort()` I realize that acwri also needs to be passing the `call` argument along.
...
url <- "https://github.com/jixing475/acwri.git"
remote <- "origin"
