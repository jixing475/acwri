...
#' unset, acwri uses [rprojroot](https://rprojroot.r-lib.org) to
...
#'
#' acwri then stores the active project for use for the remainder of the
#' session.
#'
#' In general, end user scripts should not contain direct calls to
#' `acwri::proj_*()` utility functions. They are internal functions that are
#' exported for occasional interactive use or use in packages that extend
#' acwri. End user code should call `here::here()` or other functions from
...
proj_set <- function(path = ".", force = FALSE) {
  ...
  if (is.null(new_project)) {
    ui_abort(c(
      ...
      "Read more in the help for {.fun acwri::proj_get}."
    ))
  }
...
with_project <- function(
  ...
  force = FALSE,
  setwd = TRUE,
  quiet = getOption("acwri.quiet", default = FALSE)
) {
...
local_project <- function(
  ...
  withr::local_options(acwri.quiet = quiet, .local_envir = .local_envir)
...
## acwri policy re: preparation of the path to active project
...
## acwri policy re: preparation of user-provided path to a resource on user's
## file system
user_path_prep <- function(path) {
  ## acwri uses fs's notion of home directory
...
check_is_package <- function(whos_asking = NULL) {
  ...
  if (!is.null(whos_asking)) {
    ...
    message <- c(
      "i" = "{.topic [{whos_asking}](acwri::{whos_asking_fn})} is designed to work with packages.",
      "x" = message
    )
  }
  ui_abort(message)
...
project_name <- function(base_path = proj_get()) {
  ...
  ## and DESCRIPTION is how we recognize a package as a acwri project
...
#' Activate a project
#'
#' Activates a project in the acwri, R session, and (if relevant) RStudio
