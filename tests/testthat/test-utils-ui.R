cli::test_that_cli("ui_bullets() look as expected", {
  ...
  expect_snapshot(
    ui_bullets(c(
      ...
      # other cli bullets that have no special connection to acwri history
      " " = "indent",
      "*" = "bullet",
      ">" = "arrow",
      "!" = "warning"
...
test_that("ui_bullets() respect acwri.quiet = TRUE", {
...
test_that("ui_abort() works", {
  expect_acwri_error(ui_abort("spatula"), "spatula")

  # acwri.quiet should have no effect on this
  withr::local_options(list(acwri.quiet = TRUE))
  expect_acwri_error(ui_abort("whisk"), "whisk")
})
...
test_that("acwri_map_cli() works", {
  x <- c("aaa", "bbb", "ccc")
  expect_equal(
    acwri_map_cli(x, template = "{.file <<x>>}"),
