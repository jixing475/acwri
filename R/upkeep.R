...
upkeep_checklist <- function(target_repo = NULL) {
  ...
  bullets <- c(
    todo("`acwri::use_readme_rmd()`", !file_exists(proj_path("README.Rmd"))),
    todo("`acwri::use_roxygen_md()`", !is_true(uses_roxygen_md())),
    todo("`acwri::use_github_links()`", !has_github_links),
    todo("`acwri::use_pkgdown_github_pages()`", !uses_pkgdown()),
    todo(
      "
    ...
    todo("`acwri::use_tidy_description()`"),
    todo(
      "
      `acwri::use_package_doc()`
      Consider letting acwri manage your `@importFrom` directives here. \\
      `acwri::use_import_from()` is handy for this.",
      !has_package_doc()
    ),
    todo(
      "
    ...
    todo(
      "
      `acwri::use_testthat(3)` and upgrade to 3e, \\
    ...
    todo(
      ...
      The docs for `acwri::use_r()` include a helpful script. \\
      `acwri::rename_files()` may be be useful."
    ),
    todo(
      "Consider changing default branch from `master` to `main`",
      git_default_branch() == "master"
    ),
    todo("`acwri::use_code_of_conduct()`", !has_coc()),
    todo(
    ...
    todo("`acwri::use_github_action('check-standard')`"),
    todo("`acwri::use_github_action('pkgdown')`", uses_pkgdown()),
    todo("`acwri::use_github_action('test-coverage')`", uses_testthat())
  )
...
#' `Config/acwri/last-upkeep` field in `DESCRIPTION` is consulted for this, if
...
tidy_upkeep_checklist <- function(
  ...
  bullets <- c(
    "### To begin",
    "",
    todo('`acwri::pr_init("upkeep-{format(Sys.Date(), "%Y-%m")}")`'),
    ""
  )
  ...
  if (last_upkeep <= 2000) {
    bullets <- c(
      ...
      "### Pre-history",
      "",
      todo("`acwri::use_readme_rmd()`"),
      todo("`acwri::use_roxygen_md()`"),
      todo("`acwri::use_github_links()`"),
      todo("`acwri::use_pkgdown_github_pages()`"),
      todo("`acwri::use_tidy_github_labels()`"),
  ...
  if (last_upkeep <= 2020) {
    bullets <- c(
      ...
      "### 2020",
      "",
      todo("`acwri::use_package_doc()`"),
      todo("`acwri::use_testthat(3)`"),
  ...
  if (last_upkeep <= 2022) {
    bullets <- c(
      ...
      todo('`acwri:::use_codecov_badge("{repo_spec}")`'),
      todo(
  ...
  if (last_upkeep <= 2023) {
    bullets <- c(
      ...
      todo(
        "
        Consider running `acwri::use_tidy_dependencies()` and/or \\
      ...
      todo(
        '
        `acwri::use_standalone("r-lib/rlang", "types-check")` \\
  ...
  if (last_upkeep <= 2025) {
    bullets <- c(
      ...
      "### 2025",
      "",
      todo("`acwri::use_air()` <https://posit-dev.github.io/air/>"),
      todo('`acwri::use_package("R", "Depends", "4.1")`'),
  ...
  bullets <- c(
    ...
    #
    # todo(
    #   '`acwri::use_package("R", "Depends", "{tidy_minimum_r_version()}")`',
    ...
    todo("`acwri::use_tidy_description()`"),
    todo("`acwri::use_tidy_github_actions()`"),
...
checklist_footer <- function(tidy) {
  ...
  glue(
    '<sup>\\
    Created on {Sys.Date()} with `acwri::use_{tidy_fun}upkeep_issue()`, using \\
    [acwri v{acwri_version()}](https://acwri.r-lib.org)\\
    </sup>'
  )
}

acwri_version <- function() {
  utils::packageVersion("acwri")
}
...
last_upkeep_date <- function() {
  as.Date(
    proj_desc()$get_field("Config/acwri/last-upkeep", "2000-01-01"),
    format = "%Y-%m-%d"
  )
}
...
record_upkeep_date <- function(date) {
  proj_desc_field_update("Config/acwri/last-upkeep", format(date, "%Y-%m-%d"))
}
