...
#' [use_description()]. This happens when \pkg{acwri} has to create a
#' DESCRIPTION file and the user hasn't given any author information via the
#' `fields` argument or the global option `"acwri.description"`. The
...
challenge_legacy_author_fields <- function(d = proj_desc()) {
  ...
  ui_bullets(c(
    "x" = "Found legacy {.field Author} and/or {.field Maintainer} field in
           DESCRIPTION.",
    " " = "acwri only supports modification of the {.field Authors@R} field.",
...
challenge_default_author <- function(d = proj_desc()) {
  defaults <- acwri_description_defaults()
