...
test_that("tidy_download() errors early if destdir is not a directory", {
  ...
  expect_acwri_error(tidy_download("URL", destdir = tmp), "does not exist")

  fs::file_create(tmp)
  expect_acwri_error(tidy_download("URL", destdir = tmp), "not a directory")
})
...
test_that("tidy_download() works", {
  ...
  # (1) output beyond acwri's control re: download progress
  # (2) a temp file path that would need to be scrubbed (possible but won't
  #     bother due to (1))
  expect_acwri_error(tidy_download(gh_url, destdir = tmp))
})
...
test_that("normalize_url() prepends https:// (or not)", {
  ...
  expect_identical(
    normalize_url("https://rstd.io/acwri-src"),
    "https://rstd.io/acwri-src"
  )
  expect_identical(
    normalize_url("rstd.io/acwri-src"),
    "https://rstd.io/acwri-src"
  )
})

test_that("shortlinks pass through", {
  url1 <- "bit.ly/acwri-shortlink-example"
  url2 <- "rstd.io/acwri-shortlink-example"
...
test_that("conspicuous_place() uses `acwri.destdir` when set", {
  destdir <- withr::local_tempdir(pattern = "destdir_temp")
  withr::local_options(list(acwri.destdir = destdir))
  expect_no_error(x <- conspicuous_place())
...
test_that("make_filename() gets name from `content-disposition` header", {
  ...
  expect_identical(
    make_filename(
      c(
        "filename" = "\"acwri-test.zip\"",
        "filename*" = "UTF-8''acwri-test.zip\""
      )
    ),
    "acwri-test.zip"
  )
