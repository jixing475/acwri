...
#' acwri supports HTTPS and SSH. The protocol dictates the Git URL format used
#' when acwri needs to configure the first GitHub remote for a repo:
#' * `protocol = "https"` implies `https://github.com/<OWNER>/<REPO>.git`
#' * `protocol = "ssh"` implies `git@@github.com:<OWNER>/<REPO>.git`
#'
#' Two helper functions are available:
#'   * `git_protocol()` reveals the protocol "in force". As of acwri v2.0.0,
...
git_protocol <- function() {
  protocol <- tolower(getOption("acwri.protocol", "unset"))
  ...
  options("acwri.protocol" = protocol)
  getOption("acwri.protocol")
}
...
#' # Note: If you fork-and-clone via `acwri::create_from_github()`, this is
#' # done automatically!
#'
...
git_clean <- function() {
  ...
  ui_bullets(c(
    ...
    bulletize(acwri_map_cli(ui_paths, template = "{.file <<x>>}"))
  ))
...
git_sitrep <- function(
  ...
  if ("git" %in% tool && "user" %in% scope) {
    ...
    if (!vaccinated) {
      ui_bullets(c("i" = "See {.fun acwri::git_vaccinate} to learn more."))
    }
  ...
  if (!proj_active()) {
    ui_bullets(c("i" = "No active acwri project."))
    return(invisible())
  }
  cli::cli_h2("Active acwri project: {.val {proj_get()}}")
      ...
      if (has_name(e, "db_local")) {
        ...
        ui_bullets(c(
          ...
          "!" = "The local repo has no branch named
                 {.val {e$db_source$default_branch}}.",
          "_" = "Call {.run [git_default_branch_rediscover()](acwri::git_default_branch_rediscover())} to resolve this."
        ))
