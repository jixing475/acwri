...
Specifically, the `pr_*()` functions make use of:
  ...
  * Your preferred Git transport protocol: `"https"` or `"ssh"`.
    - If acwri can't figure this out, it might ask you. You can set the
    `acwri.protocol` option to proactively address this.
...
All the code below assumes you've attached acwri in your R session:
...
The first step is to fork the source repository, to get your own copy on GitHub, and then clone that, to get your own local copy. There are many ways to accomplish these two steps, but here we demonstrate `acwri::create_from_github()`:
...
Arguments you might like to know about:
  ...
  * Use `destdir` to put praise in a specific location on your computer. You can
    set the `acwri.destdir` option if you always want acwri to put new
