...
test_that("nested package is disallowed, by default", {
  dir <- create_local_package()
  expect_acwri_error(create_package(path(dir, "abcde")), "anyway")
})

test_that("nested project is disallowed, by default", {
  dir <- create_local_project()
  expect_acwri_error(create_project(path(dir, "abcde")), "anyway")
})
...
test_that("nested package can be created if user really, really wants to", {
  ...
  withr::local_options("acwri.allow_nested_project" = TRUE)
...
test_that("we discourage project creation in home directory", {
  ...
  expect_acwri_error(create_package(path_home()), "create anyway")
  expect_acwri_error(create_project(path_home()), "create anyway")

  if (is_windows()) {
    expect_acwri_error(create_package(path_home_r()), "create anyway")
    expect_acwri_error(create_project(path_home_r()), "create anyway")
  }
})
